...
export default ComposedComponent => class WithData extends React.Component {
  ...
  static async getInitialProps (ctx) {
    ...
    if (process.browser || process.env.APOLLO_SSR_OFF) return props

    const apolloClient = initApollo({ context: pick(['res'], ctx) })
    ...
    return { ...props, __APOLLO_STATE__: apolloClient.cache.extract() }
  }

  constructor (props) {
    super(props)
    this.apollo = initApollo({ initialState: this.props.__APOLLO_STATE__ })
  }
